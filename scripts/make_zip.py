...
def  main():
  ...
  installer = "\installer\MusicNote Installer.exe"
   
  if demo:
    installer = "\installer\MusicNote Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MusicNote manual.pdf" 
  ]
  ...
  ZIPNAME = "MusicNote-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MusicNote-v" + FULLVERSIONSTR + "-win-demo.zip"
